...
"""
Tests for Doubao API functions (LLM, Image, Video generation).
"""
...
class TestDoubaoVideoGenerationRetryAsync:
    """Tests for call_doubao_video_generation_with_retry_async."""

    @pytest.fixture(autouse=True)
    def _import(self):
        from utils.generation_utils import call_doubao_video_generation_with_retry_async
        self.call_fn = call_doubao_video_generation_with_retry_async

    @pytest.mark.asyncio
    async def test_raises_when_no_api_key(self):
        with patch("utils.generation_utils.doubao_api_key", ""):
            with pytest.raises(RuntimeError, match="missing Doubao API key"):
                await self.call_fn(
    ...
    @pytest.mark.asyncio
    async def test_success_returns_video_url(self):
        """Should return video URL after successful task creation and polling."""
        create_resp = MagicMock()
        create_resp.status_code = 200
        create_resp.json.return_value = {"id": "task_123"}
        ...
        mock_http_client.post = AsyncMock(return_value=create_resp)
        mock_http_client.get = AsyncMock(return_value=poll_resp)
        mock_http_client.__aenter__ = AsyncMock(return_value=mock_http_client)
        mock_http_client.__aexit__ = AsyncMock(return_value=False)

        with patch("utils.generation_utils.doubao_api_key", "test_key"), \
             patch("utils.generation_utils.doubao_base_url", "https://ark.example.com/api/v3"), \
             patch("httpx.AsyncClient", return_value=mock_http_client):
            result = await self.call_fn(
                model_name="doubao-seedance-1-0-pro",
                prompt="A cat playing",
                config={"poll_interval": 0, "max_poll_time": 10},
                max_attempts=1,
            )
        assert result == ["https://example.com/video.mp4"]

    @pytest.mark.asyncio
    async def test_video_url_dict_format(self):
        """Should handle video_url as dict with 'url' key."""
        create_resp = MagicMock()
        create_resp.json.return_value = {"id": "task_456"}
        create_resp.raise_for_status = MagicMock()
        ...
        poll_resp.json.return_value = {
            "status": "succeeded",
            "content": [
                {"type": "video_url", "video_url": {"url": "https://example.com/v2.mp4"}}
            ],
        }
        poll_resp.raise_for_status = MagicMock()
        ...
        assert result == ["https://example.com/v2.mp4"]

    @pytest.mark.asyncio
    async def test_task_failed_retries(self):
        """Should retry when task fails."""
        create_resp = MagicMock()
        create_resp.json.return_value = {"id": "task_789"}
        create_resp.raise_for_status = MagicMock()

        poll_resp_fail = MagicMock()
        poll_resp_fail.json.return_value = {
            "status": "failed",
            "error": "GPU OOM",
        }
        poll_resp_fail.raise_for_status = MagicMock()

        mock_http_client = AsyncMock()
        mock_http_client.post = AsyncMock(return_value=create_resp)
        mock_http_client.get = AsyncMock(return_value=poll_resp_fail)
        mock_http_client.__aenter__ = AsyncMock(return_value=mock_http_client)
        mock_http_client.__aexit__ = AsyncMock(return_value=False)
    ...
    @pytest.mark.asyncio
    async def test_no_task_id_retries(self):
        """Should retry when no task ID is returned."""
        create_resp = MagicMock()
        create_resp.json.return_value = {}  # No task ID
        create_resp.raise_for_status = MagicMock()
        ...
        with patch("utils.generation_utils.doubao_api_key", "test_key"), \
            ...
            result = await self.call_fn(
                model_name="doubao-seedance-1-0-pro",
                prompt="A cat",
                config={"poll_interval": 0, "max_poll_time": 1},
                max_attempts=2,
                retry_delay=0,
            )
        assert result == ["Error"]
    ...
    async def test_auth_header_format(self):
        """Should use Bearer token auth in headers."""
        create_resp = MagicMock()
        create_resp.json.return_value = {"id": "task_999"}
        create_resp.raise_for_status = MagicMock()
        ...
        with patch("utils.generation_utils.doubao_api_key", "my_secret_key"), \
             patch("utils.generation_utils.doubao_base_url", "https://ark.example.com/api/v3"), \
             patch("httpx.AsyncClient", return_value=mock_http_client):
            await self.call_fn(
                model_name="doubao-seedance-1-0-pro",
            ...
            post_kwargs = mock_http_client.post.call_args
            headers = post_kwargs[1]["headers"] if "headers" in post_kwargs[1] else post_kwargs[0][2] if len(post_kwargs[0]) > 2 else None
            if headers is None:
                headers = post_kwargs.kwargs.get("headers", {})
    ...
    async def test_api_url_construction(self):
        ...
        create_resp = MagicMock()
        create_resp.json.return_value = {"id": "task_001"}
        create_resp.raise_for_status = MagicMock()
        ...
        with patch("utils.generation_utils.doubao_api_key", "key"), \
             patch("utils.generation_utils.doubao_base_url", "https://ark.cn-beijing.volces.com/api/v3"), \
             patch("httpx.AsyncClient", return_value=mock_http_client):
            await self.call_fn(
                model_name="doubao-seedance-1-0-pro",
                prompt="A cat",
            ...
            assert post_url == "https://ark.cn-beijing.volces.com/api/v3/contents/generations/tasks"
            # Check GET URL (poll)
            get_url = mock_http_client.get.call_args[0][0]
            assert get_url == "https://ark.cn-beijing.volces.com/api/v3/contents/generations/tasks/task_001"
